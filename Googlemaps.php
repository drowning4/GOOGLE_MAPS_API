...
class Googlemaps {
	...
	var $center						= "34.26204, -6.62241";		// Sets the default center location (lat/long co-ordinate or address) of the map. If defaulting to the users location set to "auto"
	...
	var	$markercluster				= array();					// An array used by the library to store the markersCluster as they are produced
	var	$heatmap					= array();					// An array used by the library to store the heatmap as they are produced
	...
	function add_markercluster($params = array())
	{
		
		$markercluster = array();
		
		$markercluster['points'] = array();		

		$markercluster_output = '';
		
		foreach ($params as $key => $value) {
		
			if (isset($markercluster[$key])) {
			
				$markercluster[$key] = $value;
				
			}
			
		}
		
		if (count($markercluster['points'])) {

			$markercluster_output .= '
				var markercluster_data = [';
			$i=0;
			$lat_long_output = '';
			foreach ($markercluster['points'] as $point) {
				if ($i>0) { $markercluster_output .= ','; }
				$lat_long_to_push = '';
				if ($this->is_lat_long($point)) {
					...
					$markercluster_output .= '
					new GMarker(new google.maps.LatLng('.$lat_long[0].', '.$lat_long[1].'))';
			...
			$markercluster_output .= $lat_long_output;

		}
			
		$markercluster_output .= '
			var markerCluster = new MarkerClusterer('.$this->map_name.', markercluster_data);		 
		';
		/*
		...
		*/
		array_push($this->markercluster, $markercluster_output);
	
	}
	
	
	function add_heatmap($params = array())
	{
		
		$heatmap = array();
		
		$heatmap['points'] = array();		
		$heatmap['gradient'] = array();						// The positions 
		...
		if (count($heatmap['points'])) {

			$heatmap_output .= '
				var heatmap_data = [';
			$i=0;
			$lat_long_output = '';
			foreach ($heatmap['points'] as $point) {
				if ($i>0) { $heatmap_output .= ','; }
				$lat_long_to_push = '';
				if ($this->is_lat_long($point)) {
					$lat_long_to_push = $point;
					$heatmap_output .= '
			...
			$heatmap_output .= $lat_long_output;
		...
		*/
		array_push($this->heatmap, $heatmap_output);
	...
	function add_polygon($params = array())
	{
		...
		$polygon['editable'] = FALSE;							// Defines if the polygon is editable
		...
		$polygon['id'] = count($this->polygons);				// ID of POLYGONE
		$polygon['with_delete_point'] = FALSE;						// Si with_delete_point est TRUE alor l'or du click droit le point sélectioner sera supprimer
		...
		$polygon_output .= '
			polygon_'.$polygon['id'].' = new google.maps.Polygon({
    			';
		if (count($polygon['points'])) {
			$polygon_output .= 'path: polygon_plan_'.$polygon['id'].',
					';
		}
		$polygon_output .= '
    			strokeColor: "'.$polygon['strokeColor'].'",
		...
		if ($polygon['zIndex']!="" && is_numeric($polygon['zIndex'])) {
 		...
 		$polygon_output .= '
			...
			polygon_'.$polygon['id'].'.setMap('.$this->map_name.');

		';
		if($polygon['with_delete_point'])
		{
			$polygon_output .= '
				google.maps.event.addListener(polygon_'.$polygon['id'].', "rightclick", function(evt) {
		...
		if ($polygon['onclick']!="") { 
			$polygon_output .= '
			google.maps.event.addListener(polygon_'.$polygon['id'].', "click", function() {
				'.$polygon['onclick'].'
			});
			';
		}
		if ($polygon['ondblclick']!="") { 
			$polygon_output .= '
			google.maps.event.addListener(polygon_'.$polygon['id'].', "dblclick", function() {
				'.$polygon['ondblclick'].'
			});
			';
		}
		if ($polygon['onmousedown']!="") { 
			$polygon_output .= '
			google.maps.event.addListener(polygon_'.$polygon['id'].', "mousedown", function() {
				'.$polygon['onmousedown'].'
			});
			';
		}
		if ($polygon['onmousemove']!="") { 
			$polygon_output .= '
			google.maps.event.addListener(polygon_'.$polygon['id'].', "mousemove", function() {
				'.$polygon['onmousemove'].'
			});
			';
		}
		if ($polygon['onmouseout']!="") { 
			$polygon_output .= '
			google.maps.event.addListener(polygon_'.$polygon['id'].', "mouseout", function() {
				'.$polygon['onmouseout'].'
			});
			';
		}
		if ($polygon['onmouseover']!="") { 
			$polygon_output .= '
			google.maps.event.addListener(polygon_'.$polygon['id'].', "mouseover", function() {
				'.$polygon['onmouseover'].'
			});
			';
		}
		if ($polygon['onmouseup']!="") { 
			$polygon_output .= '
			google.maps.event.addListener(polygon_'.$polygon['id'].', "mouseup", function() {
				'.$polygon['onmouseup'].'
			});
			';
		}
	...
	function create_map()
	{
		...
		if($this->heatmap){
			$apiLocation = 'https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=visualization';
		}
		else{
		...
		*/
		...
		// add markerCluster
		if (count($this->markercluster)) {
			foreach ($this->markercluster as $markercluster) {
				$this->output_js_contents .= $markercluster;
			}
		}	
		
		
		// add heatmap
		if (count($this->heatmap)) {
			foreach ($this->heatmap as $heatmap) {
				$this->output_js_contents .= $heatmap;
			}
		}	
		//
